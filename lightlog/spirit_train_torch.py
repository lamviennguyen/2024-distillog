...
value = []
value = pd.read_csv('../datasets/Spirit/vector_newgenwithfasttext.csv', header = None)
...
def read_data(path, split=0.7):
    ...
    for i in range(0, len(logs_data)):
        ...
        for j in range(0, len(data)):
            if data[j] == 378:
                padding[j] = ppa_result[data[j]-2]
            else: padding[j] = ppa_result[data[j]-1]
...
model = TCN()
model.to(device)
#summary(model, input_size=(64, 20, 50))
...
torch.save(model.state_dict(), "../datasets/Spirit/model/chronological_newgenwithfasttext_lightlog.pth")
