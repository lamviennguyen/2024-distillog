...
'''
split = 50
path = "../datasets/Spirit/chronological_test.csv"
...
def test_model(test_x, label):
    model = TCN()
    model.to(device)
    model.load_state_dict(torch.load("../datasets/Spirit/model/chronological_newgenwithfasttext_lightlog.pth",map_location=torch.device('cpu')))
    model.eval()
